...
namespace Mascot
{
    public class BaiduUnit:Notification
    {
				...
				try
				{
					...
					using (StreamReader file = File.OpenText(jsonfile))
					{
						using (JsonTextReader reader1 = new JsonTextReader(file))
						{
							...
							value = o["result"]["responses"][0]["actions"][0]["say"].ToString();
							Console.WriteLine(value);
						}
					}
					OnNoteEvent(value, new EventArgs());
					return result;
				}catch
                {
					string result = "貌似网络断开了";
					OnNoteEvent(result, new EventArgs());
					return result;
                }
			});
		}
